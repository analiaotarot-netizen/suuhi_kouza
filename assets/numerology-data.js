...
function themeReading(theme, s){
  const lp = meaningOf(s.lifePath), bd = meaningOf(s.birthday), de = meaningOf(s.destiny), so = meaningOf(s.soul), pe = meaningOf(s.personality);
  switch(theme){
    case 'love':
      return `${s.name}さんは、心の奥では${so.keyword}（ソウル${displayNumber(s.soul)}）を望んでいます。恋愛では、この望みを大事にできる相手と一緒にいると安心できるはずです。一方で周囲や相手からは${pe.keyword}（パーソナリティ${displayNumber(s.personality)}）に見えやすいので、「本当はこう思っている」と言葉にして伝えることが、より良い関係につながりそうです。`;
    case 'work':
      return `${s.name}さんは、${lp.keyword}という方向性を人生の土台に持っています（ライフパス${displayNumber(s.lifePath)}）。今の名前には${de.keyword}という役割が表れていて（ディスティニー${displayNumber(s.destiny)}）、${bd.keyword}という得意分野（バースデー${displayNumber(s.birthday)}）を活かせる働き方が向いていそうです。今の仕事や転職先を選ぶときは、「この方向性を活かせるか」を軸に考えてみるとよさそうです。`;
    case 'relationship':
      return `${s.name}さんは、周囲からは${pe.keyword}（パーソナリティ${displayNumber(s.personality)}）に見られやすいタイプです。本音の部分では${so.keyword}（ソウル${displayNumber(s.soul)}）を大事にしているので、人間関係で疲れるときは、この本音を出せる相手や場面を意識的に増やしてみると楽になりそうです。`;
    case 'talent':
      return `${s.name}さんの人生の土台は${lp.keyword}（ライフパス${displayNumber(s.lifePath)}）、生まれ持った得意分野は${bd.keyword}（バースデー${displayNumber(s.birthday)}）です。心の奥では${so.keyword}（ソウル${displayNumber(s.soul)}）を望んでいるので、この3つが重なる活動に、才能や自分らしさを感じやすいはずです。`;
    case 'money':
      return `${s.name}さんは、${lp.keyword}（ライフパス${displayNumber(s.lifePath)}）を土台に、${de.keyword}（ディスティニー${displayNumber(s.destiny)}）という役割を今の名前に持っています。お金との向き合い方も、この2つの方向性に沿った形が、${s.name}さんにとって自然な流れになりやすいでしょう。`;
    default:
      return null;
  }
}

const catLabels = { lifePath:'ライフパス', birthday:'バースデー', destiny:'ディスティニー', soul:'ソウル', personality:'パーソナリティ' };

function plainNum(v){ return (v === 11 || v === 22 || v === 33) ? `${v}／${v===11?2:v===22?4:6}` : `${v}`; }

function compatReading(a, b){
  const cats = Object.keys(catLabels);
  const same = cats.filter(k => a[k] === b[k]);
  const diff = cats.filter(k => a[k] !== b[k]);
  let text = '';
  if(same.length > 0){
    const sameDesc = same.map(k => `${catLabels[k]}が${plainNum(a[k])}`).join('・');
    text += `${a.name}さんと${b.name}さんは、${sameDesc}と一致しています。似た感覚を持ちやすく、安心して過ごせる関係を築きやすいでしょう。`;
  } else {
    text += `${a.name}さんと${b.name}さんは、5つの数字がすべて異なります。共通点よりも、お互いの違いから学び合える関係と言えそうです。`;
  }
  if(diff.length > 0){
    const pick = diff.includes('soul') ? 'soul' : diff.includes('personality') ? 'personality' : diff[0];
    const am = meaningOf(a[pick]), bm = meaningOf(b[pick]);
    text += ` 一方で${catLabels[pick]}は、${a.name}さんが${am.keyword}（${plainNum(a[pick])}）、${b.name}さんが${bm.keyword}（${plainNum(b[pick])}）と違いがあります。ここが、ぶつかりやすいポイントであると同時に、お互いにないものを補い合える魅力にもなりそうです。`;
  }
  text += ` すれ違いを感じたときは、「どちらが正しいか」ではなく「お互いの違いをどう活かせるか」を意識して話し合ってみましょう。`;
  return text;
}
